...
function parseNaturalDue(s) {
  ...
  m = s.match(/^(วันนี้|พรุ่งนี้|พรุ้งนี้|พรุงนี้)(?:\s+(เที่ยง(?:\s*ตรง)?|เที่ยงครึ่ง|เที่ยง\s*ครึ่ง|(\d{1,2})(?:[:.](\d{2}))?))?$/i);
  if (m) {
...
function parseAssignLoose(text) {
  ...
  if (!timeStr) {
    const m = body.match(/เที่ยง(?:\s*ตรง)?|เที่ยงครึ่ง|เที่ยง\s*ครึ่ง/i);
    if (m) {
